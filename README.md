...
The cluster name is derived from the filename (without directory and
extension), and the actual context names inside the files (e.g. "admin")
are not used for naming. The gathered data is stored in one directory
per cluster:
...
The cluster name is always derived from the filename, even for a single
file:
...
Use `--contexts` to select specific contexts from a kubeconfig file.
