...
The workers pick up work function from the queue and run them.

Running gather resources steps:

1. List all resources with specified type and namespaces
2. Dump every resource to the output directory
3. If an addon is registered for the resource, call addon.Inspect(). Inspecting a resource may fetch new resources from the cluster, or queue more work in the inspectWorkqueue.
4. When all workers are done, close the inspectWorkqueue
