...
`)
	...
	if configs[0].Context != "admin" {
		t.Errorf("expected context %q, got %q", "admin", configs[0].Context)
	}
	...
	if configs[1].Context != "admin" {
		t.Errorf("expected context %q, got %q", "admin", configs[1].Context)
	}
